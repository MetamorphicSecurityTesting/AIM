...
from OutputClustering.Classes.pareto_DBSCAN_outputClasses import pareto_Dbscan
...
class clustering:
    def __init__(self,refData_dir,distance_function):
        print("DBSCAN is running for Output Clustering")
        ...
        if str(distance_function).lower() in ['lev', 'levenshtein', 'levenstein']:
            clustering.Dbscan_lev(self, data, df, refData_dir)
        elif str(distance_function).lower() in ['bag']:
            clustering.Dbscan_bag(self, data, df, refData_dir)
        elif str(distance_function).lower() in ['all']:
            print("Executing DBSCAN with Levenshtein distance")
            clustering.Dbscan_lev(self, data, df, refData_dir)
            print("Executing DBSCAN with Bag distance")
            clustering.Dbscan_bag(self, data, df, refData_dir)
    ...
    def Dbscan_lev(self, data, df, refData_dir):
        ...
        clusterer = DBSCAN(eps=eps, min_samples=int(n_clusters), metric='precomputed')
        ...
        path_to_output = os.path.join(refData_dir_parent, 'Lev_DBSCAN',filename)
    ...
    def Dbscan_bag(self, data, df, refData_dir):
        ...
        # Split the file path into directory and file components
        directory, filename = os.path.split(file_path)
        path_to_output = os.path.join(refData_dir_parent, 'Bag_DBSCAN',filename)
        df.to_csv(path_to_output, sep='\t', encoding='utf-8', header='true')
        print("Output clustering successfully terminated.")
        print("The result is written in: " + str(path_to_output))
