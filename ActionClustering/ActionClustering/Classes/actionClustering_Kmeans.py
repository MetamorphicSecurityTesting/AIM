...
from ActionClustering.Classes import Main
from ActionClustering.Classes.pareto_Kmeans_actionSet import pareto_kmeans_action
import numpy as np
import pandas as pd
...
class urlDist:
    ...
    def __init__(self,refined_data,OutputClasses_dir):
        ...
        print("K-means is running for Action Clustering")
    ...
    def actionSet(self,refined_data,df,OutputClasses_dir):
        ...
        for i in range (max(df['labels'])+1):
            ...
            if len(actionSet_post) > 1:
                urlDist.Kmeans(self, actionSet_post)

            if len(actionSet_get) > 1:
                urlDist.Kmeans(self, actionSet_get)

            if len(actionSet_post) == 1:
                actionSet_post['SubClass'] = [int(0)]
                # Print the cluster labels
                # self.df_final = pd.concat([self.df_final, actionSet_post])
                urlDist.sortLabels(self, actionSet_post)

            if len(actionSet_get) == 1:
                actionSet_get['SubClass'] = [int(0)]
                # Print the cluster labels
                # self.df_final = pd.concat([self.df_final, actionSet_get])
    ...
    def Kmeans(self,actionSet):
        ...
        if distances.any() == np.zeros((len(data), len(data))).any():
            ...
            n_clusters = 90
            resF, resX = pareto_kmeans_action.init(self,distances)
            if resF != [[]]:
                a = np.array(resF[:,0])
