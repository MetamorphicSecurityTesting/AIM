...
class pareto_Dbscan():
    ...
    def init(self,  distance_matrix):
        ...
        problem = ProblemWrapper(n_var=2, n_obj=2, xl=[1, 3], xu=[2, 10])
    ...
    def benchmarks_Dbscan(self, n, eps):
        ...
        num_unique_items = len(unique_elements)
        if len(distance_matrix[0]) == num_unique_items and num_unique_items != 2:
            clusterer = DBSCAN(eps=eps+1, min_samples=int(n), metric='precomputed')
            # Fit the clusterer to the distance matrix
            clusterer.fit(distance_matrix)
            # Get the cluster labels for each datapoint
            labels = clusterer.labels_
        ...
        if labels.any() == np.zeros(len(labels)).any():
            ...
            if labels.__len__() == 2:
                silhouette_avg = 0
                sample_silhouette_values = np.array([0.0, 0.0])
            else:
